...
This model suggests that infection decreases in bigger fish, and that there is no effect of crowding. Overdispersion seems mild. The ratio of residual deviance to residual degrees of freedom is about 1.4, so we would expect the results to be unaffected by using `family = 'quasibinomial'` argument.
